...
const start=str('start',5),end=str('end',5),status=str('status',20),note=str('note',160);if(!/^([01]\d|2[0-3]):[0-5]\d$/.test(start)||!/^([01]\d|2[0-3]):[0-5]\d$/.test(end)||start>=end)throw new TypeError('Revisá el horario de inicio y fin.');

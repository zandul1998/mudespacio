...
import type {Product,Schedule} from './shared';
...
export async function listSchedules(admin=false){const db=bindings().DB;if(!db)throw Error('Database unavailable');const {results}=await db.prepare(admin?'SELECT * FROM schedules ORDER BY day, sort, start':'SELECT * FROM schedules WHERE published=1 ORDER BY day, sort, start').all<Schedule>();return results}

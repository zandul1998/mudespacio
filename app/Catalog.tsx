...
if(products===null)return <div className="catalog-loading" role="status" aria-label="Cargando productos">{[0,1,2].map(i=><span key={i}/>)}</div>;
if(!products.length)return <div className="catalog-empty"><div><h3>¿Querés conocer nuestra merch?</h3><p>Escribinos y te contamos qué tenemos disponible para encargar.</p></div><a className="button" href={wa('¡Hola Mud! Quisiera conocer la merch disponible y sus precios.')} target="_blank" rel="noreferrer">Consultar merch</a></div>;
